...
rclcpp_action::ResultCode common_resultcode =
    rclcpp_action::ResultCode::UNKNOWN;
int common_action_result_code =
    control_msgs::action::FollowJointTrajectory_Result::SUCCESSFUL;

// todo(johntgz) fine tune desired_goals
std::vector<double> desired_goals = {0.0, -1, 1, 0.0};
...
void common_feedback(
    ...
    const std::shared_ptr<
        ...
        feedback) {
  std::cout << "feedback->desired: effort: " << feedback->desired.effort.at(0);
  std::cout << ", velocities: " << feedback->desired.velocities.at(0)
            << std::endl;

  std::cout << "feedback->actual: effort: " << feedback->actual.effort.at(0);
  std::cout << std::endl;

  if (ct_goals_reached < desired_goals.size()) {
    if (fabs(feedback->actual.effort[0] - desired_goals.at(ct_goals_reached)) <
        0.1) {
      std::cout << "Goal # " << ct_goals_reached << ": "
                << desired_goals.at(ct_goals_reached) << " reached"
                << std::endl;
      ct_goals_reached++;
      if (ct_goals_reached < desired_goals.size()) {
        std::cout << "next goal # " << ct_goals_reached << ": "
                  << desired_goals.at(ct_goals_reached) << std::endl;
      }
    }
  }
}
...
int main(int argc, char* argv[]) {
  ...
  RCLCPP_INFO(node->get_logger(),
              "Creating JointTrajectoryPoint msg with effort goals");

  std::vector<trajectory_msgs::msg::JointTrajectoryPoint> points;

  // todo(johntgz) remove all effort feedforward values
  trajectory_msgs::msg::JointTrajectoryPoint point;
  point.positions.clear();
  point.time_from_start = rclcpp::Duration::from_seconds(1.0);
  point.effort.resize(joint_names.size());
  point.effort[0] = desired_goals[0];
  // Need to add position here or it will fail the message validation checks
  // within JointTrajectoryController
  // For effort command interfaces, JointTrajectoryController still requires
  // desired position
  ...
  point2.effort.resize(joint_names.size());
  point2.effort[0] = desired_goals[1];
  ...
  point3.effort.resize(joint_names.size());
  point3.effort[0] = desired_goals[2];
  ...
  point4.effort.resize(joint_names.size());
  point4.effort[0] = desired_goals[3];
  ...
  rclcpp_action::Client<
      control_msgs::action::FollowJointTrajectory>::SendGoalOptions opt;
  opt.goal_response_callback =
      std::bind(common_goal_response, std::placeholders::_1);
  opt.result_callback =
      std::bind(common_result_response, std::placeholders::_1);
  opt.feedback_callback =
      std::bind(common_feedback, std::placeholders::_1, std::placeholders::_2);
